...
export default function useHeaderRotator() {
    useEffect(() => {
        try {
            ...
            const urls = attr.split(',').map(s => s.trim()).filter(Boolean);
            if (urls.length < 2) return;

            const intervalAttr = parseInt(header.getAttribute('data-interval') || '2000', 10);
            const intervalMs = Number.isFinite(intervalAttr) && intervalAttr > 0 ? intervalAttr : 2000;
            const gradient = 'linear-gradient(rgba(0, 0, 0, 0.4), rgba(0, 0, 0, 0.4))';

            header.classList.add('header-rotator', 'show-a', 'kb-a');

            urls.forEach(u => { const img = new Image(); img.src = u.replace(/^url\((.*)\)$/, '$1').replace(/^"|"$/g, ''); });
